# WARNING: NodeBB is moving from Docker Hub to GitHub Container Registry. This means that this image will no longer be maintained. In order to continue receiving updates pleace change the image tag from `nodebb/docker` to `ghcr.io/nodebb/nodebb`.
