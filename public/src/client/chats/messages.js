...
define("forum/chats/messages", [
	...
	"translator",
	"benchpress",
], function (components, sounds, translator, Benchpress) {
	var messages = {};
	...
	messages.appendChatMessage = function (chatContentEl, data) {
		var lastSpeaker = parseInt(
			chatContentEl.find(".chat-message").last().attr("data-uid"),
			10
		);
		var lasttimestamp = parseInt(
	...
	function onMessagesParsed(chatContentEl, html) {
		...
		if (
			...
			chatContentEl[0].scrollHeight - chatContentEl.height() * 1.5
		) {
	...
	messages.delete = function (messageId, roomId) {
		translator.translate("[[modules:chat.delete_message_confirm]]", function (
			translated
		) {
